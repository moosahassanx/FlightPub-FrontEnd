...
const Groups = () => {
    ...
    const localStorarageMarkers = JSON.parse(localStorage.getItem("user-markers")) || []
    ...
    async function handleClick() {
        ...
        if(!newUserGroup.some(e => e.username === userInput) && userInput !== ''){
                    ...
                    <h2> Group List Container</h2>
                        ...
                        <div id="list-group" className="row justify-content-center">
                                <ListGroup variant="flush">
                                    {                               
                                        userGroup?.map((users) => <ListGroupItem action onClick={() => handleClick1(users.username)}>{users.username}</ListGroupItem>)
                                    }
                                </ListGroup>
                        </div>
                    <Form>
                        <div id="email-row-form" className="row">
                            <Form.Control type="email" placeholder="Enter your friends email" onChange={e => setUserInput(e.target.value)} value={userInput}></Form.Control>
                        </div>
                        <div id="button-row-form" className="row">
                                <div className="col-lg-6">      
                                    <Button variant="primary" size="lg" onClick={handleClick} block>Add</Button>               
                                </div>
                                <div className="col-lg-6">
                                    <Button variant="danger" size="lg" onClick={handleRemove} block>Remove</Button>
                                </div>
                        </div>
                    </Form> 
                </div>
                ...
                <div id="map-col" className="col-md-8">
                    <GoogleMap
                        ...
                        key="AIzaSyDE4QqTEwYOVTFexogb-Aj7KWkex3ize4A">
                                <Marker key="AIzaSyDE4QqTEwYOVTFexogb-Aj7KWkex3ize4A" position={{lat: 34, lng: 123}}   />     
