...
const SearchForm = () => {
                ...
                if(num === 1)
                {
                    getFlightAvailblity(item).then(available=>{ 
                        ...
                        {   
                            ...
                            if(!found){
                                let temp = airlines
                                temp.push({name: item.airlineCode.airlineName , checked: true})
                                setAirlines(temp)
                            }
                        }
                        })
    ...
    const handleSubmit = (event)=>{
        ...
        if(tripType === 'One-Way' && searchType === 'Flexible' && (depStartDate === null || depEndDate === null))
        {
            alert("The date range must be selected");
            error = true;
        } else if(tripType === 'Return' && searchType === 'Flexible' && (depStartDate === null || depEndDate === null || reStartDate === null || reEndDate === null))
        {
            alert("The date range must be selected");
            error = true;
        }
    ...
    //  }, [filtersObj, filters])
     ...
     const handleFilters = (e) =>{
        if(e.target.name === 'noStop'){ 
            ...
            } else {        //otherwise they should be added from the original list
    ...
    return(
        <>
            ...
            {search 
                &&
                <div className="container-fluid text-center">
                    <FlightCard oneWayData ={filteredFlights} returnData={filteredReFlights} trip ={tripType} numberOfTravellrs ={numberOfTravellers} tType ={tClass}/>
                </div>
            }
        </>
    )
